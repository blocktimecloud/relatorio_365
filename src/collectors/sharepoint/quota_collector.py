...
class SharePointQuotaCollector:
    ...
    def _parse_resposta(data: dict) -> dict:
        ...
        total_mb = sum(e.get("TenantStorageMB", 0) for e in entradas)
        usado_mb = sum(e.get("GeoUsedStorageMB", 0) for e in entradas)
        disponivel_mb = sum(e.get("GeoAvailableStorageMB", 0) for e in entradas)
